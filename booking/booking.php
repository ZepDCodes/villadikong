...
// ✅ NEW: Fetch booked date ranges to disable them on the calendar
// We are grabbing all confirmed bookings and passing them to JavaScript
$booked_query = $conn->query("SELECT checkin, checkout FROM bookings WHERE status != 'Cancelled'");
$booked_ranges = [];
...
$booked_dates_json = json_encode($booked_ranges);
?>
...
<script>
  // ✅ Grab the booked dates from PHP
  const disabledDates = <?php echo $booked_dates_json; ?>;

  // ✅ Initialize Checkout Picker first so we can update it from the Checkin Picker
  ...
  flatpickr("#checkin", {
      ...
      onChange: function(selectedDates, dateStr, instance) {
          // When a checkin date is selected, force the checkout minimum date 
          // to be the day after check-in.
